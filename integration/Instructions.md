...
- Fixes dashboard panels that failed with `Unknown column [total_ecu]` / `[conf_ecu_rate]` on 0.3.1 by dual-writing legacy ECU field names on lookup indices. After upgrade, add legacy field mappings to existing lookup indices or reset those transforms (restart alone does not update mappings). Delete and re-import the Chargeback dashboard saved objects if Kibana did not replace them—see [troubleshooting](docs/troubleshooting.md).
